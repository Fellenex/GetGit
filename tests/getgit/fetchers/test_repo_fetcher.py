...
from getgit.fetchers import RepoFetcher
...
def test_self_path_uses_user_repos_with_owner_affiliation():
    ...
    out = RepoFetcher(client).list_repos("me", is_self=True)
...
def test_stranger_path_uses_users_username_repos():
    ...
    RepoFetcher(client).list_repos("alice", is_self=False)
...
def test_returns_list_not_iterator():
    ...
    out = RepoFetcher(client).list_repos("x", is_self=False)
