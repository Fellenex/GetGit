...
def test_file_extension_simple():
    ...
    assert PullRequestFetcher._file_extension("foo.py") == ".py"
    assert PullRequestFetcher._file_extension("path/to/foo.yml") == ".yml"


def test_file_extension_compound():
    """Only the last suffix is considered (so `.tar.gz` → `.gz`)."""
    assert PullRequestFetcher._file_extension("archive.tar.gz") == ".gz"
...
def test_file_extension_falls_back_to_basename_for_extensionless_files():
    ...
    assert PullRequestFetcher._file_extension("Dockerfile") == "Dockerfile"
    assert PullRequestFetcher._file_extension("path/Makefile") == "Makefile"


def test_file_extension_dotfiles_are_their_own_key():
    """Hidden files like `.gitignore` should bucket under their bare name, not ''."""
    assert PullRequestFetcher._file_extension(".gitignore") == ".gitignore"


def test_extract_jira_codes_groups_by_project_prefix():
    """Codes should bucket under their project prefix."""
    out = PullRequestFetcher._extract_jira_codes("WD-1 YWFB-2 WD-3")
...
def test_extract_jira_codes_dedupes_across_inputs():
    ...
    out = PullRequestFetcher._extract_jira_codes("WD-1 and YWFB-2", "WD-1 again", "PTR-99")
...
def test_extract_jira_codes_ignores_none_and_empty():
    ...
    assert PullRequestFetcher._extract_jira_codes(None, "", "WD-5") == {"WD": ["WD-5"]}


def test_extract_jira_codes_is_deterministic_across_input_orders():
    """Output should not depend on input order."""
    a = PullRequestFetcher._extract_jira_codes("WD-1 YWFB-2 PTR-3")
    b = PullRequestFetcher._extract_jira_codes("PTR-3 YWFB-2 WD-1")
...
def test_extract_jira_codes_requires_uppercase_prefix():
    ...
    assert PullRequestFetcher._extract_jira_codes("wd-1 and Pr-2") == {}


def test_extract_jira_codes_no_match_returns_empty_dict():
    """No matches anywhere should produce an empty dict (not None)."""
    assert PullRequestFetcher._extract_jira_codes("nothing here", "still nothing") == {}
