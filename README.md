...
| `github/` | Everything GitHub-specific, layered `service → provider → client`: `GithubClient` — the **only** owner of GitHub routes and transport, exposing one typed endpoint method per route (`list_own_repos`, `search_issues`, `get_pull_request`, …) — plus `GithubSettings` and the `RateLimitExceededError`/`RepositoryAccessError` exceptions (`clients/`); the wire-shape response objects the client returns (`RepoSummary`, `IssueSearchResult`, `PullRequestDetail`, `PullRequestFile`, `PullRequestReview`, `Comment`, `CommitPayload`) alongside the internal `Commit`/`PullRequest`/`Review` domain models and the `AuthorshipReport`/`GithubScrapeResult` aggregates (`data/`); `GithubProvider`, which unravels response objects into domain objects (`providers/`); and `GithubService`, the facade over client + provider (`services/`). |
