...
**Decision:** every class lives in its own file named after it (`AppSettings` → `app_settings.py`). Files are grouped into domain folders under `src/getgit/`: `application/`, `authentication/`, `cli/`, `exporting/`, `fetchers/`, `github_api/`, `models/`, `services/`. Each domain has an `__init__.py` that re-exports its public types so external callers do `from getgit.fetchers import PullRequestFetcher` instead of digging into module paths. Tests mirror the same layout (`tests/getgit/<domain>/test_<file>.py`).
