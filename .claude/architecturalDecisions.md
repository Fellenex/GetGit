...
If a prior decision is reversed or extended, the original entry is annotated with a `**Reversed YYYY-MM-DD:**` or `**Updated YYYY-MM-DD:**` note pointing at the newer entry — entries are never silently overwritten.
...
### 2026-05-13 — 403 from GitHub locks the client and aborts the scrape
...
### 2026-05-13 — Consolidate GitHub-specific code into `github/` with `clients/`, `providers/`, `data/`
...
### 2026-05-13 — `services/` domain — `GithubService` facade over per-resource fetchers
...
### 2026-05-13 — `data/` subdomain inside mixed-content domains
...
### 2026-05-13 — `exporting/` domain with a `Writer` protocol
...
### 2026-05-13 — `GithubSettings` replaces `Auth`/`PersonalTokenAuth`
...
### 2026-05-13 — `application/` domain — UI-agnostic orchestration
